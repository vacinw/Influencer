...
const CreatorDashboard = () => {
 ...
 // Fetch True Total Applicants from our new endpoint
 useEffect(() => {
 ...
 console.error('Failed to fetch true total applicants', err);
 }
 };
 fetchApplicantStats();
 }, []);
 ...
 <div>
 <h1 className="text-2xl font-bold text-gray-900">Bảng Điều Khiển Creator</h1>
 <p className="text-sm text-gray-500">Quản lý các chiến dịch và tìm kiếm các ứng viên phù hợp</p>
 </div>
 <div>
 ...
 <dl>
 <dt className="text-sm font-medium text-gray-500 truncate">Tổng Chiến Dịch</dt>
 <dd>
 ...
 <span className="font-medium text-indigo-600">Xem hiệu suất</span>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="ml-5 w-0 flex-1">
 <dl>
 <dt className="text-sm font-medium text-gray-500 truncate">Khám Phá Users</dt>
 <dd>
 <div className="text-lg font-medium text-gray-900">Tìm Kiếm</div>
 </dd>
 </dl>
 </div>
 </div>
 <div className="bg-gray-50 px-5 py-3 mt-5 -mx-5 -mb-5">
 <div className="text-sm">
 <Link to="/explore" className="font-medium text-indigo-600 hover:text-indigo-500">Tìm Ngay</Link>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="ml-5 w-0 flex-1">
 <dl>
 <dt className="text-sm font-medium text-gray-500 truncate">Tổng Ứng Viên</dt>
 <dd>
 ...
 <div className="bg-gray-50 px-5 py-3 mt-5 -mx-5 -mb-5">
 <div className="text-sm">
 <Link to="/creator/all-applicants" className="font-medium text-indigo-600 hover:text-indigo-500">Xem Phân Tích</Link>
 </div>
 </div>
 </div>
 </div>
 ...
 <h3 className="text-lg leading-6 font-medium text-gray-900">Chiến Dịch Của Bạn</h3>
 ...
 className="pl-9 pr-4 py-2 border border-gray-300 rounded-md text-sm focus:border-black"
 />
 </div>
 ...
 Hông tìm thấy chiến dịch nào hợp lệ.
 </div>
 ) : (
 <>
 ...
 <p className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${campaign.status === 'Đang tuyển' || campaign.status === 'Active'
 ? 'bg-green-100 text-green-800'
 : 'bg-gray-100 text-gray-800'
 }`}>
 {campaign.status === 'Active' || campaign.status === 'Đang tuyển' ? 'Đang Tuyển' : campaign.status === 'CLOSED' ? 'Đã Đóng' : campaign.status === 'COMPLETED' ? 'Đã Hoàn Thành' : campaign.status}
 </p>
 ...
 <span className="font-bold text-indigo-600 mr-1">{campaign.applicantCount || 0}</span> Ứng Viên
 </p>
 </div>
 <div className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
 <Calendar className="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400" aria-hidden="true" />
 <p>Đóng vào <time dateTime={campaign.deadline}>{new Date(campaign.deadline).toLocaleDateString()}</time></p>
 </div>
 </div>
 </div>
 </div>
 ...
 Đang xem trang <span className="font-medium">{page + 1}</span> / <span className="font-medium">{totalPages}</span>
 </p>
 </div>
 <div>
 ...
 <h2 className="text-lg leading-6 font-medium text-gray-900">Công Việc & Mốc Thời Gian Đang Chạy</h2>
 ...
 <button onClick={handleRefreshJobs} className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-md" title="Làm mới">
 ...
 ? "Không có công việc nào thoã bộ lọc."
 : "Chưa có công việc nào. Ứng viên đang chờ bạn ứng tuyển đó!"}
 </div>
 ) : (
 ...
 <span className="flex items-center mr-3 truncate">
 <Users size={14} className="mr-1 flex-shrink-0" />
 <span className="truncate">{job.influencer?.fullName || job.influencer?.username}</span>
 </span>
 ...
 Sửa từ {new Date(job.createdAt).toLocaleDateString()}
 </div>
 </div>
 </div>
 <div className="flex flex-row items-center justify-end gap-3 flex-shrink-0 w-64">
 <span className={`px-2.5 py-0.5 inline-flex whitespace-nowrap justify-center text-xs leading-5 font-semibold rounded-full min-w-[120px] ${job.status === 'COMPLETED' ? 'bg-green-100 text-green-800' :
 job.status === 'IN_PROGRESS' ? 'bg-blue-100 text-blue-800' :
 'bg-gray-100 text-gray-800'
 }`}>
 {job.status === 'COMPLETED' ? 'Hoàn Thành' : job.status === 'IN_PROGRESS' ? 'Đang Thực Hiện' : job.status.replace('_', ' ')}
 </span>
 <button
 onClick={() => navigate(`/job/${job.id}`)}
 className="inline-flex items-center justify-center whitespace-nowrap px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none shadow-sm transition-colors w-[100px]"
 >
 Quản lý
 </button>
 </div>
