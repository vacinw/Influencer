                    ...
                    <div className="flex">
                        ...
                        <div className="hidden sm:ml-10 sm:flex sm:space-x-8">
                            ...
                            {user?.role?.name === 'CREATOR' && (
                                <>
                                    <Link to="/creator/dashboard" className="border-transparent text-gray-500 hover:text-gray-900 inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium">Bảng Điều Khiển</Link>
                                    <Link to="/wallet" className="border-transparent text-gray-500 hover:text-gray-900 inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium">Ví</Link>
                                </>
                            )}
                            {user?.role?.name === 'RECEIVER' && (
                                <>
                                    <Link to="/receiver/dashboard" className="border-transparent text-gray-500 hover:text-gray-900 inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium">Bảng Điều Khiển</Link>
                                    <Link to="/wallet" className="border-transparent text-gray-500 hover:text-gray-900 inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium">Ví</Link>
                                </>
                            )}
                            {user?.role?.name === 'ADMIN' && (
                                <Link to="/admin/dashboard" className="border-transparent text-gray-500 hover:text-gray-900 inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium">Quản Trị</Link>
                            )}
                        </div>
                    </div>
                                        ...
                                        isProfileOpen && (
                                            <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none animate-in fade-in zoom-in-95 duration-100">
                                                <div className="px-4 py-2 border-b border-gray-100 text-xs text-gray-500">
                                                    Đăng nhập dưới tên <br /> <strong className="text-gray-900">{user?.email}</strong>
                                                </div>
                                                ...
                                                <Link to="/settings" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Cài Đặt</Link>
                                                <button
            ...
            {isMenuOpen && (
                <div className="sm:hidden bg-white border-t border-gray-100">
                    <div className="pt-2 pb-3 space-y-1">
                        <Link to="/" className="bg-indigo-50 border-indigo-500 text-indigo-700 block pl-3 pr-4 py-2 border-l-4 text-base font-medium">Trang Chủ</Link>
                        <Link to="/explore" className="border-transparent text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800 block pl-3 pr-4 py-2 border-l-4 text-base font-medium">Khám Phá</Link>
                    </div>
