...
const WalletPage = () => {
 ...
 if (location.state?.autoDepositAmount) {
 setDepositAmount(location.state.autoDepositAmount.toString());
 ...
 alert("Trạng thái: Chưa nhận được giao dịch nạp tiền. Vui lòng đợi thêm vài giây và thử lại.");
 }
 } catch (error) {
 console.error("Failed to fetch wallet data manually", error);
 alert("Lỗi khi kiểm tra. Vui lòng tắt form nạp và tự F5 lại.");
 }
 };
 ...
 if (curr.status === 'FAILED' || curr.status === 'CANCELLED') {
 return acc;
 }
 ...
 try {
 await api.put('/wallet/bank-info', {
 bankName,
 bankAccountName,
 bankAccountNumber
 });
 ...
 alert(error.response?.data || "Cập nhật thông tin thất bại");
 } finally {
 ...
 <CreditCard className="text-indigo-600" size={20} /> Thông Tin Ngân Hàng
 </h3>
 {!isEditingBankInfo ? (
 <button
 ...
 >
 {savingBankInfo ? 'Đang lưu...' : 'Lưu Thay Đổi'}
 </button>
 </div>
 )}
 </div>
 ...
 <div 
 className="w-full px-3 py-2 border border-gray-300 rounded-lg focus-within:ring-1 focus-within:ring-indigo-500 focus-within:border-indigo-500 text-sm bg-white cursor-pointer flex items-center justify-between transition-all"
 onClick={() => setIsBankDropdownOpen(!isBankDropdownOpen)}
 >
 {bankName ? (
 <div className="flex items-center gap-2">
 {vietQRBanks.find(b => b.shortName === bankName || b.name === bankName)?.logo ? (
 <img src={vietQRBanks.find(b => b.shortName === bankName || b.name === bankName)?.logo} alt="logo" className="w-6 h-6 object-contain" />
 ...
 <ChevronDown size={16} className={`transition-transform ${isBankDropdownOpen ? 'rotate-180' : ''} text-gray-400`} />
 </div>
 
 {isBankDropdownOpen && (
 <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl max-h-64 overflow-y-auto animate-in fade-in zoom-in-95 duration-100">
 ...
 <div 
 key={bank.id}
 className="flex items-center gap-3 px-3 py-2.5 hover:bg-indigo-50 cursor-pointer border-b border-gray-50 last:border-0 transition-colors"
 onClick={() => {
 setBankName(bank.shortName);
 setIsBankDropdownOpen(false);
 }}
 >
 <img src={bank.logo} alt={bank.shortName} className="w-12 h-8 object-contain bg-white rounded border border-gray-100 px-1 py-0.5" />
 <div>
 <p className="text-sm font-bold text-gray-900 leading-tight">{bank.shortName} <span className="text-xs font-normal text-gray-500 ml-1">({bank.code})</span></p>
 <p className="text-[10px] text-gray-500 line-clamp-1 mt-0.5">{bank.name}</p>
 </div>
 </div>
 ))
 )}
 ...
 <div className="flex items-center gap-2">
 {bankName && vietQRBanks.find(b => b.shortName === bankName || b.name === bankName)?.logo && (
 <img src={vietQRBanks.find(b => b.shortName === bankName || b.name === bankName)?.logo} alt="logo" className="w-6 h-6 object-contain" />
 )}
 <p className="font-medium text-gray-900">{bankName || <span className="text-gray-400 italic">Chưa cập nhật</span>}</p>
 </div>
 )}
 </div>
 <div>
 <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Tên Chủ Tài Khoản</label>
 {isEditingBankInfo ? (
 <input
 type="text"
 value={bankAccountName}
 onChange={(e) => setBankAccountName(e.target.value)}
 className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-black text-sm"
 placeholder="VD: NGUYEN VAN A"
 />
 ) : (
 <p className="font-medium text-gray-900">{bankAccountName || <span className="text-gray-400 italic">Chưa cập nhật</span>}</p>
 )}
 </div>
 <div>
 ...
 {bankAccountNumber ? `****${bankAccountNumber.slice(-4)}` : <span className="text-gray-400 italic tracking-normal">Chưa cập nhật</span>}
 </p>
 )}
 </div>
 </div>
 ...
 className="block w-full pl-8 pr-12 py-3 text-xl font-bold text-gray-900 border-2 border-gray-100 rounded-xl focus:border-black outline-none transition-all"
 placeholder="10,000"
 />
 ...
 className="block w-full pl-7 pr-12 py-3 sm:text-sm border-gray-300 rounded-xl focus:border-black border"
 placeholder="0.00"
 autoFocus
 />
