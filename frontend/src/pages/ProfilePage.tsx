...
const ProfilePage = () => {
 ...
 try {
 const isSelf = Boolean((!profileId) || (initialUser && initialUser.id.toString() === profileId));
 setIsOwnProfile(isSelf);

 const [profileRes, statusRes] = await Promise.all([
 isSelf ? api.get('/users/profile') : api.get(`/users/${profileId}/public`),
 isSelf ? api.get('/verification/my-status').catch(() => ({ data: [] })) : Promise.resolve({ data: [] })
 ]);
 ...
 {isOwnProfile && isEditing && !isUploadingCover && (
 ...
 {isOwnProfile && isEditing && !isUploading && (
 <label className="absolute inset-0 bg-black/50 flex items-center justify-center cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity">
 ...
 {user?.verified || user?.isVerified || verificationStatus === 'APPROVED' ? (
 ...
 <div className="text-center">
 <p className="text-sm text-gray-600 mb-4">Người dùng này chưa có trạng thái xác minh rõ ràng.</p>
 </div>
 )}
 </div>
 </div>
